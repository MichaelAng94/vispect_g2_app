 ...
 */
public class DrivingVideosActivity extends BaseActivity {

    private final static String TAG = "DrivingVideosActivity";
    ...
    private Runnable notmoreRunnable = new Runnable() {
        @Override
        public void run() {
            XuToast.show(DrivingVideosActivity.this, STR(R.string.cube_views_load_not_date));
            switch (isrefresh) {
                case 0:
    ...
    private Runnable mRunnable_toas = new Runnable() {
        @Override
        public void run() {
            XuToast.show(DrivingVideosActivity.this, STR(R.string.download_fail));
    ...
    private Runnable onFielError = new Runnable() {
        ...
        public void run() {
            //文件损坏
            DialogHelp.getInstance().hideDialog();
            XuToast.show(DrivingVideosActivity.this, STR(R.string.file_corrupted));
        }
    };
    ...
    private Runnable mRunnable_toas_waiitingtoolong = new Runnable() {
        ...
        public void run() {
            //时间过长的话就提示让用户手动连
            if (canshowlongtime) {
                DialogHelp.getInstance().connectDialog(DrivingVideosActivity.this, STR(R.string.wifi_waiting_too_long) + AppConfig.getInstance(DrivingVideosActivity.this).getWifi_name() + STR(R.string.wifi_waiting_too_long2) + AppConfig.getInstance(DrivingVideosActivity.this).getWifi_paw());
                //   showProgress(true, STR(R.string.wifi_waiting_too_long) + AppConfig.getInstance(DrivingVideosActivity.this).getWifi_name() + STR(R.string.wifi_waiting_too_long2) + AppConfig.getInstance(DrivingVideosActivity.this).getWifi_paw());
            }
    ...
    protected void initView(View view) {
        ...
        CheckTime.toStop();
//        findViewById(R.id.iv_titlebar_mune).setOnClickListener(new OnClickListener() {
//            @Override
//            public void onClick(View v) {
//                showPopupWindow(setting_menu, -1);
//            }
//        });
//        XuView.setViewVisible(findViewById(R.id.iv_titlebar_mune), true);
        initlistview();
    ...
    void getVideos(int pageIndex) {
        timeoututil.startCheck(ARG.SET_VALUE_TIMEOUT);
        AppContext.getInstance().getDeviceHelper().getDVRLists(pageIndex, pageSize, new DrivingVideoOperationListener() {
            @Override
            public void onGetVideoList(final ArrayList arrayList) {
                timeoututil.stopCheck();
                mHandler.post(new Runnable() {
            ...
            public void onLast() {
                timeoututil.stopCheck();
                XuToast.show(DrivingVideosActivity.this, STR(R.string.cube_views_load_more_loaded_empty));
                switch (isrefresh) {
                    case 0:
                        if (setting_menu != null) {
                            setting_menu.hideHeaderView();
    ...
    void showPopupWindow(View parent, final int position) {
        ...
        if (!adapter.isIsedit()) {
            contentView = LayoutInflater.from(DrivingVideosActivity.this).inflate(R.layout.popupwindow_drivingvideos_menu, null);
        } else {
            contentView = LayoutInflater.from(DrivingVideosActivity.this).inflate(R.layout.popupwindow_drivingvideos_menu2, null);
        }
...
//        });
        ...
        contentView.findViewById(R.id.ll_popuwindow_lock).setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                // TODO Auto-generated method stub
                popupWindow.dismiss();
                DialogHelp.getInstance().connectDialog(DrivingVideosActivity.this, "Wait");
...
//                                        getnow();
        ...
        contentView.findViewById(R.id.ll_popuwindow_savetophone).setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (isdownloading || !XuString.isEmpty(AppContext.getInstance().getDownloading_name())) {
                    XuToast.show(DrivingVideosActivity.this, STR(R.string.please_wai_last_file_down_done));
                    return;
                }
                DialogHelp.getInstance().connectDialog(DrivingVideosActivity.this,"Save");
    ...
    private void download(int position) {
        ...
        DialogHelp.getInstance().connectDialog(DrivingVideosActivity.this,"Download");
        switch (AppContext.getInstance().getDeviceHelper().getCommunicationType()) {
            case 0:
                openDownloadOld(position);
                break;
                    ...
                    public void run() {
                        ...
                        if (downloading_position >= 0) {
                            ...
                            if (checkList != null && checkList.size() > 0) {
                                ...
                                Toast.makeText(DrivingVideosActivity.this, "完成", Toast.LENGTH_SHORT).show();
                                ;
                            }
                        }
                    }
    ...
    void openUploadServiceAndDownload(final int postion, final String value) {
        ...
        AppContext.getInstance().getDownThreadExecutor().execute(new Runnable() {
            ...
            public void run() {
                ...
                mHandler.removeCallbacks(mRunnable_toas_waiitingtoolong);
                mHandler.post(mRunnable);
                AppContext.getInstance().getDeviceHelper().downloadDVR(value, progressCallback);
